...
var (
	errBufTooSmall = errors.New("the buffer is too small to contain a single message")
	filenameRegexp = regexp.MustCompile("^chunk([0-9]+)$")
)
...
func (s *OnDisk) getFileDescriptor(chunk string) (*os.File, error) {
	...
	fp, err := os.OpenFile(filepath.Join(s.dirname, chunk), os.O_CREATE|os.O_RDWR, 0o666)
	if err != nil {
